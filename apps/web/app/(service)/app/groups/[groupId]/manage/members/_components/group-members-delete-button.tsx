...
interface GroupMembersDeleteButtonProps {
  userIds: number[];
}

export default function GroupMembersDeleteButton({
  userIds,
}: GroupMembersDeleteButtonProps) {
  const [openAlert, setOpenAlert] = React.useState(false);
  const [isLoading, setIsLoading] = React.useState(false);
  return (
    <AlertDialog open={openAlert} onOpenChange={setOpenAlert}>
      <AlertDialogTrigger className="text-semantic-warning">
        삭제
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogTitle>선택한 멤버를 삭제할까요?</AlertDialogTitle>
          <AlertDialogDescription>
            {userIds.length}명의 정보가 모두 삭제됩니다.
          </AlertDialogDescription>
        </AlertDialogHeader>
